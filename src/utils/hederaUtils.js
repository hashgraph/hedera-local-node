...
module.exports = class HederaUtils {
  static async importFees (host = '127.0.0.1') {
    ...
    const queryFees = new HederaSDK.FileContentsQuery().setFileId(
      `0.0.${feesFileId}`
    );
    ...
    const queryExchangeRates = new HederaSDK.FileContentsQuery().setFileId(
      `0.0.${exchangeRatesFileId}`
    );
    const exchangeRates = (await queryExchangeRates.execute(client)).toString(
      'hex'
    );
    await shell.exec(

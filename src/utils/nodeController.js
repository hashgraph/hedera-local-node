  ...
  static async startLocalNode (argv) {
    ...
    console.log(
      `Starting hedera local node in ${
        multiNode ? 'multi-node mode' : 'single-node mode'
      }...`
    );
    shell.cd(__dirname);
    shell.cd('../../');
  ...
  static async applyConfig (
    ...
    if (multiNode) {
      NodeController.setEnvValue(
        ...
        '{"network-node:50211":"0.0.3","network-node-1:50211":"0.0.4","network-node-2:50211":"0.0.5","network-node-3:50211":"0.0.6"}'
      );
    }
    if (result.code !== 0) {
    ...
    if (!fullMode || multiNode) {
      ...
      const application = yaml.load(
        fs.readFileSync(
          `${baseFolder}/compose-network/mirror-node/application.yml`
        )
      );
      if (!fullMode) {
        application.hedera.mirror.importer.dataPath = 'file:///node/';
        application.hedera.mirror.importer.downloader.sources = [
          { type: 'LOCAL' }
        ];
      }
      if (multiNode) {
      ...
      fs.writeFileSync(
        `${baseFolder}/compose-network/mirror-node/application.yml`,
        yaml.dump(application, { lineWidth: 256 })
      );
    }
  }
